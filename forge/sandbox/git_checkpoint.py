...
Lifecycle:
    setup()          → create forge/working branch from HEAD
...
class GitCheckpoint:
    ...
    async def setup(self) -> str:
        ...
        if await self._is_dirty():
            ...
            await self._run(
                "commit", "-m", "forge: auto-save before agent session",
                "--allow-empty",
            )
    ...
    async def checkpoint(self, message: str) -> str | None:
        ...
        await self._run("add", "-A")
        await self._run("commit", "-m", f"forge: {message}")
    ...
    async def finalize(self) -> str | None:
        ...
        await self._run("commit", "-m", "forge: apply agent changes")
