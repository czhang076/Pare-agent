...
# Feature flag plumbing — the flat ReAct loop (R3) runs in parallel with the
# legacy 3-layer orchestrator path. ``--loop new`` or ``PARE_USE_NEW_LOOP=1``
# activates ``run_headless_flat_react`` below. The default flips to ``new``
# in R4 and ``PARE_USE_LEGACY_LOOP=1`` becomes the opt-out until R5 deletes
# the legacy path entirely.
def _flat_react_requested(cli_value: str | None) -> bool:
    if cli_value in ("new", "flat", "react"):
        return True
    if cli_value in ("legacy", "old"):
        return False
    env = os.environ.get("PARE_USE_NEW_LOOP", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Flat ReAct entry point (R3) — parallel path, opt-in via PARE_USE_NEW_LOOP=1
# ---------------------------------------------------------------------------
...
async def run_headless_flat_react(
    ...
    *,
    ...
    """Flat ReAct headless runner — R3 parallel path to :func:`run_headless`.
    ...
    as a v1/v2 :class:`TrajectoryRecord`. Exit code mirrors ``run_headless``:
    ``0`` on ``LoopResult.success``, ``1`` otherwise, ``2`` on setup errors.

    The existing :func:`run_headless` stays untouched so R4 can flip the
    default cleanly and R5 can delete the legacy path in one step.
    """
